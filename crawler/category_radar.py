...
# ✅ 카테고리 사전(원하면 계속 튜닝)
# 규칙: title에 아래 키워드가 포함되면 해당 카테고리 점수 +1
CATEGORY_RULES = {
...
def classify(title: str):
    """
    멀티라벨 가능하지만, 운영에서는 '대표 1개'가 편함:
    - 가장 많이 매칭된 카테고리를 대표로 선정
    - 매칭이 없으면 '기타'
    """
...
def main():
    ...
    counts = Counter()
    examples = defaultdict(list)  # cat -> [(rank,title,url)]
    for it in items:
    ...
    # 텔레그램: 변화가 있거나(Δ≠0) 정기 요약을 보내고 싶을 때
